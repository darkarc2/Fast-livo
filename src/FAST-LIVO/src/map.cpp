...
namespace lidar_selection
{
    ...
    bool Map::safeDeleteFrame(FramePtr frame)   //安全删除帧
    {
        bool found = false;
        for (auto it = keyframes_.begin(), ite = keyframes_.end(); it != ite; ++it)
        {
            if (*it == frame)
            {//对于匹配的关键帧，删除其所有特征点的引用
    ...
    void Map::delete_points(int size)
    {
        ...
        // }
        // print_line;
        map_points_.erase(map_points_.begin(), map_points_.begin() + size - 10); //为什么要-10
        values.erase(values.begin(), values.begin() + size - 10);
        // print_line;
        //}
    }

    void Map::clear()//map 清除
    {
        // int size = map_points_.size();
        // int map_threshold = 20000;
    ...
    {
        for (auto kf : keyframes_)//遍历所有关键帧
        {
            // check if kf has overlaping field of view with frame, use therefore KeyPoints
            for (auto keypoint : kf->key_pts_)//遍历所有关键点
            {
                if (keypoint == nullptr)
                    continue;

                if (frame->isVisible(keypoint->point->pos_))//这个关键点是否在这个帧中可视
                {
                    close_kfs.push_back(
                        std::make_pair(
                            kf, (frame->T_f_w_.translation() - kf->T_f_w_.translation()).norm()));//打包成pair<FramePtr,double>，放到close_kfs中
    ...
    // 对于getCloseKeyframes拿到的所有靠近的关键帧，排序拿到距离最近的关键帧
    FramePtr Map::getClosestKeyframe(const FramePtr &frame) const
    {
        ...
        // Sort KFs with overlap according to their closeness//匿名函数进行排序
