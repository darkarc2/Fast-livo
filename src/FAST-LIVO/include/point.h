...
public:
  ...
  bool                        normal_set_;              //!< Flag whether the surface normal was estimated or not.//标记是否估算了表面法线。
  list<FeaturePtr>              obs_;                     //!< References to keyframes which observe the point. //观测的关键帧
  size_t                      n_obs_;                   //!< Number of observations: Keyframes AND successful reprojections in intermediate frames.
  int                         last_published_ts_;       //!< Timestamp of last publishing.
  int                         last_projected_kf_id_;    //!< Flag for the reprojection: don't reproject a pt twice.//重投影flag
  // PointType                   type_;                    //!< Quality of the point.
  int                         n_failed_reproj_;         //!< Number of failed reprojections. Used to assess the quality of the point.，重投影失败次数
  ...
  //高博书，公式7.48，优化特征点的空间位置，观测相机方程关于特征点的两个导数矩阵。
